...
const Form_Mesas_Jurados = ({ onClose, edit, onNuevoClick }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Crear un objeto payload con los datos que deseas enviar
      const payload = {
        eleccion: formData.eleccion,
            ...
            <Grid container spacing={2}>
              <Grid item xs={12} sm={12} md={12} lg={12}>
                <FormControl fullWidth variant="outlined">
                  <InputLabel htmlFor="eleccion" sx={{ color: "primary.main" }}>
                    Elección
                  </InputLabel>
                  <Select
                    value={formData.eleccion}
                    onChange={(e) =>
                      setFormData({ ...formData, eleccion: e.target.value })
                    }
                    label="Eleccion"
                  >
                    {elecciones.map((eleccionItem) => (
                      <MenuItem
                        key={eleccionItem.nombre}
                        value={eleccionItem.nombre}
                      >
                        {eleccionItem.nombre}
                      </MenuItem>
                    ))}
              ...
              <Grid item xs={12} sm={12} md={12} lg={12}>
                <FormControl fullWidth variant="outlined">
                  <InputLabel htmlFor="facultad" sx={{ color: "primary.main" }}>
                    Facultad
                  </InputLabel>
                  <Select
                  ...
                  >
                    {elecciones
                      .find(
                        (eleccionItem) =>
                          eleccionItem.nombre === formData.eleccion
                      )
                      ?.facultades.map((facultadItem) => (
                        <MenuItem
                          key={facultadItem.nombre}
...
export default Form_Mesas_Jurados;

Form_Mesas_Jurados.propTypes = {
